...
class TranslationModel(Model_Wrapper):
    ...
    def __init__(self, params, model_type='Translation_Model', verbose=1, structure_path=None, weights_path=None,
        ...
        if params['SRC_PRETRAINED_VECTORS'] is not None:
            ...
            src_word_vectors = {}
            with open(os.path.join(params['SRC_PRETRAINED_VECTORS'])) as f, open('bbc/embedding', 'rb') as ef:
                structured_embedding = np.load(ef)
                for line in f:
                    values = line.split()
                    word = values[0]
                    coefs = np.asarray(values[1:], dtype='float32')
                    src_word_vectors[word] = coefs
            ...
            for word, index in self.vocabularies[self.ids_inputs[0]]['words2idx'].iteritems():
                ...
                w = w.lower()
                eid = int(eid)
                if src_word_vectors.get(w) is not None:
                    self.src_embedding_weights[index, :] = np.append(src_word_vectors[w], structured_embedding[eid, :])
                else:
                    print 'Unknown source word', word
        ...
        if params['TRG_PRETRAINED_VECTORS'] is not None:
            ...
            trg_word_vectors = {}
            with open(os.path.join(params['TRG_PRETRAINED_VECTORS'])) as f:
                for line in f:
                    values = line.split()
                    word = values[0]
                    coefs = np.asarray(values[1:], dtype='float32')
                    trg_word_vectors[word] = coefs
            ...
            for word, index in self.vocabularies[self.ids_outputs[0]]['words2idx'].iteritems():
                if trg_word_vectors.get(word) is not None:
                    ...
                    print 'Unknown target word', word
